...
const PLACEMAKERS_CATALOGUE = [
  ...
  },
  {
    ...
    category: 'Kitchens & Laundry > Laundry > Tapware',
  ...
  // ── WET AREA LININGS & WATERPROOFING ──────────────────────────────────────
  {
    ...
    category: 'Building Products > Plasterboard > Wet Area Linings',
  ...
  },
  {
    ...
    category: 'Timber & Plywood > Framing',
  ...
  },
  {
    ...
    category: 'Fasteners & Fixings > Screws > Decking Screws',
...
async function seed() {
  ...
  console.log('Generating embeddings for PlaceMakers items...');
  const chunks = PLACEMAKERS_CATALOGUE.map((p) => {
  ...
  try {
    const res = await openai.embeddings.create({
      ...
      input: chunks.map(c => c.slice(0, 4000)),
    });
    embeddings = res.data.map(d => d.embedding);
    console.log('Embeddings ready!');
  } catch (err) {
    console.warn('OpenAI embedding error, using fallback:', err.message);
    embeddings = chunks.map(() => new Array(1536).fill(0));
  }

  for (let i = 0; i < PLACEMAKERS_CATALOGUE.length; i++) {
    const p = PLACEMAKERS_CATALOGUE[i];
    const chunk = chunks[i];
    const embedding = embeddings[i];

    // 1. Insert/Update in products collection
    const productRecord = {
      projectId: 'placemakers',
      parentSku: p.sku,
      brandCode: 'PlaceMakers NZ',
      name: p.name,
      category: p.category,
      description: p.description,
      images: [p.imageUrl],
      swatchImages: [p.imageUrl],
      priceUSD: { min: p.price, max: p.price, cost: p.price },
      rating: { value: 4.9, count: 88 },
      source: 'placemakers-nz-catalogue',
      ...
      sourceUrl: p.url,
      taxonomy: {
        garmentGroup: p.category,
      },
      stockInfo: {
        label: 'In Stock · PlaceMakers Mt Wellington (60-Min Pickup)',
        color: '#059669',
      },
    };

    await prodsCol.updateOne(
      { projectId: 'placemakers', parentSku: p.sku },
      { $set: productRecord },
      { upsert: true }
    );

    // 2. Insert/Update in documents collection
    const docRecord = {
      brand: 'placemakers',
      projectId: 'placemakers',
      type: 'product',
    ...
    await docsCol.updateOne(
      { projectId: 'placemakers', 'metadata.sku': p.sku },
      { $set: docRecord },
      { upsert: true }
    );

    console.log(`[${i + 1}/${PLACEMAKERS_CATALOGUE.length}] Saved [${p.sku}] ${p.name}`);
  }

  console.log('\nAll PlaceMakers catalogue products and RAG documents successfully indexed!');
  await client.close();
}
