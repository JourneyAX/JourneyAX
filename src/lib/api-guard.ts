...
export async function guard<T = unknown>(
  ...
  // Key the limit on the session when there is one. A school, a call centre
  // and an office all share a single public IP, so an IP-keyed limit punishes
  // the second person through the door rather than the abuser.
  const session = sessionFromRequest(req);
  const key = session ? `${scope}:sid:${session.sub}` : clientKey(req, scope);
