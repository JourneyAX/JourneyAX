// Virtual try-on provider — composites a garment onto a shopper's uploaded photo.
// Provider-abstracted: prefers Gemini 2.5 Flash Image ("nano banana") when a
// GEMINI_API_KEY is configured (best identity/pose preservation); otherwise falls
// back to OpenAI gpt-image-1 multi-image edit (the key we ship with today).
//
// Positioning guardrail: this is a VISUALIZATION ("see how it may look"), never a
// guarantee of physical fit. Callers must label the output as an AI preview.
...
function dataUrlToBuffer(dataUrl: string): { buf: Buffer; mime: string } {
  ...
  if (!m) throw new Error('personImage must be a base64 data URL');
...
export function tryOnProvider(): 'gemini' | 'openai' | null {
...
export async function generateTryOn(input: TryOnInput): Promise<TryOnResult> {
  ...
  if (!provider) throw new Error('No image model key configured (set GEMINI_API_KEY or OPENAI_API_KEY).');
  ...
  if (provider === 'gemini') return geminiTryOn(person, garment, prompt);
  return openaiTryOn(person, garment, prompt);
}

async function openaiTryOn(person: { buf: Buffer; mime: string }, garment: { buf: Buffer; mime: string }, prompt: string): Promise<TryOnResult> {
...
async function geminiTryOn(person: { buf: Buffer; mime: string }, garment: { buf: Buffer; mime: string }, prompt: string): Promise<TryOnResult> {
  ...
  if (!r.ok) throw new Error(`gemini image failed: ${r.status} ${(await r.text()).slice(0, 200)}`);
  ...
  if (!inl?.data) throw new Error('gemini returned no image');
