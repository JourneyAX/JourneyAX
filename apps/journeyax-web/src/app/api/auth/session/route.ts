/**
 * Storefront BFF session. Returns the signed-in customer's profile if the
 * HttpOnly access cookie is valid — lets the app restore its session on load
 * WITHOUT exposing a token to JS. Middleware has already refreshed a
 * near-expiry access cookie before this runs.
 */
